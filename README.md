...
- [sudoswap-analysis](#sudoswap-analysis)
  - [sudoswap OTC](#sudoswap-otc)
    - [解析](#解析)
      - [order](#order)
        - [makerAssetData 和 takerAssetData](#makerassetdata-和-takerassetdata)
      - [AssetProxy](#assetproxy)
      - [订单成交](#订单成交)
    - [例子](#例子)
      - [1. 调用 fillOrder 方法进行成单](#1-调用-fillorder-方法进行成单)
      - [2. 成单后发出 Fill 事件](#2-成单后发出-fill-事件)
      - [3. 需要注意的两点：](#3-需要注意的两点)
        - [1. 具体买卖信息要从 makerAssetData 和 takerAssetData 中获取。](#1-具体买卖信息要从-makerassetdata-和-takerassetdata-中获取)
        - [2. 判断 0xv2 上的交易是否是 sudoswap OTC 交易主要通过 feeRecipientAddress 是否是 0x4e2f98c96e2d595a83AFa35888C4af58Ac343E44](#2-判断-0xv2-上的交易是否是-sudoswap-otc-交易主要通过-feerecipientaddress-是否是-0x4e2f98c96e2d595a83afa35888c4af58ac343e44)
  - [sudoswap AMM](#sudoswap-amm)
    ...
    - [如何获取某个 NFT 的所有流动池](#如何获取某个-nft-的所有流动池)
      - [1. 监听 LSSVMPairFactory 合约的 NewPair 事件](#1-监听-lssvmpairfactory-合约的-newpair-事件)
    ...
    - [如何获取某个 NFT 的交易数据](#如何获取某个-nft-的交易数据)
      - [例子](#例子-1)
        - [解析](#解析-2)
          - [1. 流动池地址](#1-流动池地址)
          - [2. 获取流动池信息](#2-获取流动池信息)
          - [2. 分析 Transfer 事件](#2-分析-transfer-事件)
          - [2. 分析 event 类型](#2-分析-event-类型)
          - [3. SpotPriceUpdate](#3-spotpriceupdate)
          - [4. 计算用户获取的 eth](#4-计算用户获取的-eth)
    - [获取某个 NFT 的交易数据的其他办法](#获取某个-nft-的交易数据的其他办法)
      - [1. 使用 Defined](#1-使用-defined)
      - [2. 使用 Reservoir](#2-使用-reservoir)
  - [参考](#参考)
